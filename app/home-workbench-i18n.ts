...
const ko: HomeWorkbenchMessage = {
  ...
  hero: {
    ...
    description:
      "현재 공개 범위는 공식 홈페이지와 사용자 실행형 AIWORK Browser RC입니다. 선택 AI·문서·메일·실행을 잇는 Workbench는 후속 제품 비전이며, 외부 작업은 사용자 승인을 전제로 설계합니다.",
  ...
  workbench: {
    ...
    actions: ["리서치 → 서명 포함 PDF", "웹페이지 입력·파일 업로드", "이메일 초안·단건 발송"],
  ...
  capabilities: {
    ...
    cards: [
      ...
      {
        title: "Daum Email·Gemma4",
        description: "안전 경계 계약만 있으며 네트워크 클라이언트·모델 실행기는 연결되지 않았습니다.",
        status: "비활성 계약만",
      },
      {
        title: "Agent Runtime·외부 실행",
        description: "메일 발송·웹 입력·파일 업로드·화면 직접 조작은 아직 구현되지 않았습니다.",
        status: "미구현 · 후속 단계",
      },
    ],
  },
  boundary: {
    ...
    advancedSummary: "후속 실행 엔진 상태",
    advancedBody:
      "GitHub 저장소에는 후속 통합 계약과 설계 문서가 포함되어 있지만 Agent Runtime, OpenClaw Gateway, Daum 연결, Gemma4 추론, Desktop Helper 직접 조작은 운영 기능으로 활성화되지 않았습니다.",
  },
  cta: {
    eyebrow: "AIWORK",
...
const en: HomeWorkbenchMessage = {
  ...
  hero: {
    ...
    description:
      "The current public scope is the AIWORK website and user-invoked Browser RC. A Workbench that connects chosen AI, documents, email, and execution remains the product vision.",
  ...
  workbench: {
    ...
    actions: ["Research → signed PDF", "Web input and file upload", "Email draft and one-shot send"],
  ...
  capabilities: {
    ...
    cards: [
      ...
      { title: "Daum Email & Gemma4", description: "Safety contracts exist, but no network client or model runtime is connected.", status: "Disabled contracts only" },
      { title: "Agent Runtime & actions", description: "Email sending, web input, uploads, and direct screen control are not implemented.", status: "Not implemented" },
    ],
  },
  boundary: {
    eyebrow: "CLEAR BOUNDARIES",
    ...
    advancedSummary: "Future execution-engine status",
    advancedBody:
      "The GitHub repository contains future integration contracts and design documents, but Agent Runtime, OpenClaw Gateway, Daum connectivity, Gemma4 inference, and Desktop Helper control are not enabled production features.",
  },
  cta: {
    eyebrow: "AIWORK",
...
const ja = localizeFromEnglish({
  ...
  status: "中核基盤の実装完了 · 実サービス接続を段階的に検証中",
...
const zhCN = localizeFromEnglish({
  ...
  taskLabels: ["调研 → 带签名的 PDF", "网页填写与文件上传", "邮件草稿与单封发送"],
...
const ar = localizeFromEnglish({
  ...
  status: "اكتملت البنية الأساسية · يجري التحقق من ربط الخدمات على مراحل",
  ...
  capabilityLabels: [
    "سياق علامات تبويب متعددة",
    "مقارنة المستندات",
    "البحث إلى PDF",
    "مسودة بريد وإرسال واحد",
    "إدخال بيانات الويب ورفع الملفات",
    "مراجعة الشاشة والتحكم المباشر",
  ],
  taskLabels: ["البحث إلى PDF موقّع", "إدخال بيانات الويب ورفع الملفات", "مسودة بريد وإرسال واحد"],
...
const es = localizeFromEnglish({
  ...
  status: "Base principal implementada · conexiones reales en validación gradual",
  ...
  capabilityLabels: [
    "Contexto de varias pestañas",
    "Comparación de documentos",
    "Investigación → PDF",
    "Borrador y envío único",
    "Entrada web y carga",
    "Revisión y control de pantalla",
  ],
  taskLabels: ["Investigación → PDF firmado", "Entrada web y carga de archivos", "Borrador y envío único"],
...
const fr = localizeFromEnglish({
  ...
  status: "Socle principal implémenté · connexions réelles validées par étapes",
  ...
  capabilityLabels: [
    "Contexte multi-onglets",
    "Comparaison de documents",
    "Recherche → PDF",
    "Brouillon et envoi unique",
    "Saisie web et import",
    "Vérification et contrôle écran",
  ],
  taskLabels: ["Recherche → PDF signé", "Saisie web et import de fichiers", "Brouillon et envoi unique"],
...
const de = localizeFromEnglish({
  ...
  status: "Zentrale Grundlage umgesetzt · reale Verbindungen werden schrittweise geprüft",
  ...
  capabilityLabels: [
    ...
    "Dokumentenvergleich",
    "Recherche → PDF",
    "E-Mail-Entwurf & Einzelversand",
    "Webeingabe & Upload",
    "Bildschirmprüfung & direkte Steuerung",
  ],
  taskLabels: ["Recherche → signiertes PDF", "Webeingabe & Datei-Upload", "E-Mail-Entwurf & Einzelversand"],
